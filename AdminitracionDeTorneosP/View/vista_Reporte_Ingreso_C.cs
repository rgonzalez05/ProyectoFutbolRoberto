...
namespace AdminitracionDeTorneosP.View
{
    public partial class vista_Reporte_Ingreso_C : Form
    {
        ...
        private void cargarDataGrid()
        {
            ...
            string query = $"EXEC REPORTE_INGRESOS_C '{fechaInicio}', '{fechaFinal}'";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    listaIngresos.DataSource = data;
                    connection.Close();
                }
                catch (Exception error)
                {
                    MessageBox.Show(error.Message, "No se pudieron obtener los registros");
                }
            }
        }
